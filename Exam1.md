...
این متن کاملاً تمیز، ساختاریافته و آمادهٔ استفاده در محیط‌های آموزشی یا تکلیف است. اگر بخواهید، می‌توانم حل کامل سوالات را نیز ارائه دهم.
...
برای حل این تمرین SVM به‌صورت گام‌به‌گام پیش می‌رویم. فرض می‌شود که هسته خطی (Linear kernel) به‌کار می‌رود و ضرایب α_i در جدول همان ضرایب بهینهٔ حاصل از حل مسئلهٔ دوگان SVM هستند.

گام 1: استخراج بردارهای پشتیبان (Support Vectors)

ضرایب α_i که مقدار غیرصفر دارند، نشان‌دهندهٔ SVها هستند:

x1=(8,5.8), y1=+1, α1=0.414  
x4=(5,2), y4=−1, α4=0.018  
x7=(7,8), y7=+1, α7=0.018  
x9=(4,4.2), y9=−1, α9=0.414

پس مجموعهٔ SV = {x1, x4, x7, x9} است.

گام 2: محاسبهٔ بردار وزن w و bias b

برای kernel خطی داریم:

w = Σ_i α_i y_i x_i
b = y_k − w·x_k  (برای هر SV k)

محاسبهٔ w:

w_x1 = α1 y1 x1_x + α4 y4 x4_x + α7 y7 x7_x + α9 y9 x9_x  
       = 0.414×1×8 + 0.018×(−1)×5 + 0.018×1×7 + 0.414×(−1)×4  
       = 3.312 − 0.09 + 0.126 − 1.656 = 1.692

w_x2 = α1 y1 x1_y + α4 y4 x4_y + α7 y7 x7_y + α9 y9 x9_y  
       = 0.414×1×5.8 + 0.018×(−1)×2 + 0.018×1×8 + 0.414×(−1)×4.2  
       = 2.4012 − 0.036 + 0.144 − 1.7388 = 0.7704

پس w ≈ (1.692, 0.7704)

محاسبهٔ b (با استفاده از SV x1):

w·x1 = 1.692×8 + 0.7704×5.8 ≈ 13.536 + 4.468 = 18.004  
b = y1 − w·x1 = 1 − 18.004 = −17.004

بنابراین تابع تصمیم h(x) عبارت است از:

h(x) = w·x + b = 1.692 x_1 + 0.7704 x_2 − 17.004

پاسخ بخش (الف):

تابع تصمیم SVM:

h(x) = 1.692 x_1 + 0.7704 x_2 − 17.004

که با استفاده از ضرایب α_i و SVها به‌دست آمد.

پاسخ بخش (ب): فاصلهٔ x6 تا ابرصفحه

نقطهٔ x6=(3.8,3.8), y6=−1

h(x6)=1.692×3.8 + 0.7704×3.8 − 17.004
      = 6.4296 + 2.9275 − 17.004 ≈ −7.647

اندازهٔ w:

|w| = √(1.692² + 0.7704²) ≈ √(2.862 + 0.5935) ≈ √3.4555 ≈ 1.858

مارژین برای x6:

δ6 = y6 · h(x6) / |w| = (−1) × (−7.647) / 1.858 ≈ 7.647 / 1.858 ≈ 4.116

چون α6=0، این نقطه SV نیست؛ همچنین |h(x6)|≈7.647 > 1، لذا خارج از margin قرار دارد.

پاسخ بخش (ب): فاصلهٔ x6 تا ابرصفحه ≈ 4.116 و این نقطه SV نیست.

پاسخ بخش (پ): طبقه‌بندی نقطهٔ (3,3)

h((3,3)) = 1.692×3 + 0.7704×3 − 17.004
          = 5.076 + 2.3112 − 17.004 ≈ −9.617

چون h((3,3)) < 0، نقطه در کلاس −1 طبقه‌بندی می‌شود.

خلاصهٔ پاسخ‌ها

الف) h(x)=1.692 x_1+0.7704 x_2−17.004  
ب) فاصلهٔ x6 تا ابرصفحه ≈ 4.116؛ SV نیست.  
پ) h((3,3)) ≈ −9.617 ⇒ نقطه در کلاس −1 قرار می‌گیرد.
